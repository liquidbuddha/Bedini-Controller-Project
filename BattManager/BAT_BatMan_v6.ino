...
*/
   ...
   */  
    ...
    // VARIALBLES used by program
      ...
      long  LogTimeDuration = LogDuration  * 60000;  // convertion from millis to seconds
      
      long  TimeNow = millis();
      long  TimeStop = TimeNow + BattSwapFullDur;
      long  TimeLog = TimeNow + LogTimeDuration;
      long  Time2Log = (TimeLog - TimeNow)/60000;
      long  TimetoSwap = (TimeStop - TimeNow)/60000;
      int coilValue;
      int Threshold;
      int RPM;
      int CurrentValue;
  ...
  //  double alpha = .9;    // leaky ingtegration method variable, to smooth batt readings
    ...
    */
    ...
    void checkBatts(){
        ...
        temp=analogRead(ChargeBatt);  // read the voltage for SOURCE
        Serial.println(temp/40.92);
        ChargeBattvin = temp/40.92;

        temp=analogRead(SourceBatt);  // read the voltage for SOURCE
        SourceBattvin = temp/40.92;
...
// string looks like this:    time,CoilValue, Threshold,RPM,Current,BattStatus,SourceBattVolt,ChrBattVolt 
//                         ie 2,0,30,5389,13,0,4.34;4.37;


          dataString += String(millis()/1000/60);
          dataString += ","; 

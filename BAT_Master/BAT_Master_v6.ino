  ...
  */
  ...
  // VARIABLES  - Startup DEFAULTS
    ...
    int    MasterUIRefreshRate   = 250;        //  delay in microseconds before the LCD is refreshed.
  ...
  // VARIALBLES used by sketch
    ...
    char Mode = 'A';                       // Declares the opening MODE
  ...
  */
       ...
       if (LR < 480)     {
             PulseDelay = (abs(PulseDelay + (LR-480)/100))+1;     // reduced by factor of 10, abssolute = keep it positive
          }
        else if (LR > 520) {
             PulseDelay = PulseDelay + ((LR-520)/100);
          }

  ...
  // VARIABLES  - Startup DEFAULTS
    long UIRefreshRate = 9900;                   //  default is 9900 (< 1 s) | variable of itterations before  master is updated. 
                                                 //  Also used for PPM (# Pulses per minute) calculation
  
  // ************************************************************************************************
   
  // VARIALBLES used by program
    int       Threshold         = 10;                   //  sets variable for the Threshold value, manipulates coil sensitivity)
    int       PulseDelay        = 6000 ;
    int       Current           = 0 ;                   //  sets variable for the Current - manipulate pulse length in miliseconds
    ...
    long      CycleCounter      = 0 ;                   //  used to calc pulse width
    int       PulseCounter      = 0 ;
  ...
  void SensorCoilCheck() {  
    ...
    if (Mode == 'A') {
        if (coilValue >= Threshold ){                   // if coilValue is greater or equal to threshold
              ...
              //  if the Delay has lapsed and the drives are off, then turn them on
              if (CurrentTime >= EndOfDelay && DriveCoilState != true) {  
                  // -------------------------------------------------- COIL ON 
                  digitalWrite(DriveCoil01, HIGH);
...
//       Serial.print("..setting values..");     
        Cmdata.etCurrentValue  = CycleDuration;
